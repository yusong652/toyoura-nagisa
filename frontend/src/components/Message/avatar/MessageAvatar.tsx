import React from 'react'
import { MessageAvatarProps } from '../types'

/**
 * Message avatar display component.
 * 
 * Displays appropriate avatar image based on sender type with hover tooltip support.
 * Handles avatar loading and positioning for both user and bot messages.
 * 
 * Args:
 *     sender: Message sender type ('user' | 'bot')
 *     onMouseEnter: Optional mouse enter handler for tooltip display
 *     onMouseLeave: Optional mouse leave handler for tooltip cleanup
 * 
 * Returns:
 *     JSX element with avatar image and event handlers
 */
const MessageAvatar: React.FC<MessageAvatarProps> = ({ 
  sender, 
  onMouseEnter, 
  onMouseLeave 
}) => {
  const avatarSrc = sender === 'user' ? '/user-avatar.jpg' : '/Nagisa_avatar.jpg'
  const altText = sender === 'user' ? 'User' : 'Nagisa'
  
  return (
    <img 
      src={avatarSrc}
      alt={altText}
      className="avatar"
      onMouseEnter={onMouseEnter}
      onMouseLeave={onMouseLeave}
    />
  )
}

export default MessageAvatar