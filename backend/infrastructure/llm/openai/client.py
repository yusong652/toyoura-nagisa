...
from backend.config import get_system_prompt, get_llm_settings
...
class OpenAIClient(LLMClientBase):
    """
    OpenAI GPT 客户端实现 - 基于统一架构重构
    
    采用与Gemini/Anthropic客户端相同的模块化架构设计，
    支持流式响应、工具调用、多模态内容等全部功能特性。
    ...
    Components:
    - OpenAIContextManager: 上下文和消息历史管理
    - ResponseProcessor: 响应处理和工具调用提取
    - ToolManager: MCP工具集成和管理
    - Content Generators: 专用内容生成工具
    """
    ...
    async def get_response(
        ...
        **kwargs
        ...
        """
        流式OpenAI API调用 - 与Gemini/Anthropic架构完全对齐
    ...
    def _generate_execution_id(self) -> str:
        """生成唯一执行ID"""
        return f"OPENAI_{str(uuid.uuid4())[:8]}"
    ...
    async def _streaming_tool_calling_loop(
        ...
        **kwargs
        ...
        """
        流式工具调用循环 - 与Gemini/Anthropic完全对齐的核心实时通知引擎
        ...
        # 获取初始响应
        openai_messages = context_manager.get_working_messages()
