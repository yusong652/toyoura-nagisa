"""
Shared image prompt generator implementation.

Common text-to-image prompt generation logic that can be adapted for different LLM providers.
"""
...
from ...base.content_generators import BaseImagePromptGenerator
from ..utils.text_processing import extract_text_content, parse_text_to_image_response, enhance_prompts_with_defaults
from ..utils.text_to_image import load_text_to_image_history, save_text_to_image_generation
from ..constants.defaults import DEFAULT_FEW_SHOT_MAX_LENGTH, DEFAULT_CONTEXT_MESSAGE_COUNT, DEFAULT_NEGATIVE_PROMPT
from ..constants.prompts import DEFAULT_TEXT_TO_IMAGE_SYSTEM_PROMPT, CONVERSATION_TEXT_PROMPT_PREFIX


class SharedImagePromptGenerator(BaseImagePromptGenerator):
    """
    Shared implementation of image prompt generation.
    
    Provides common logic for text-to-image prompt generation that can be adapted
    by different LLM providers. Each provider needs to implement the actual API call.
    """
    ...
    @staticmethod
    def build_messages_for_generation(context: Dict[str, Any]) -> list:
        """
    ...
    def process_generation_response(
        ...
        text_prompt, negative_prompt = enhance_prompts_with_defaults(
            ...
            default_positive_prompt=context.get('default_positive_prompt', ''),
            default_negative_prompt=context.get('default_negative_prompt', ''),
            debug=debug
        )
