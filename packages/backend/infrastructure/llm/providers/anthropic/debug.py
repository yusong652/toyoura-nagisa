...
class AnthropicDebugger:
    ...
    @staticmethod
    def log_api_payload(api_kwargs: Dict[str, Any], component: str = "ANTHROPIC", detailed: bool = True) -> None:
        """
    ...
    def log_api_call_info(
        tools_count: int, model: str, thinking_level: Optional[str] = None, component: str = "AnthropicClient"
    ) -> None:
        """
        ...
        Args:
            ...
            thinking_level: Thinking level (default, low, high)
            component: Component name for log prefix
        """
        ...
        if thinking_level and thinking_level != "default":
            print(f"[{component}] Thinking level: {thinking_level}")

    @staticmethod
    def print_debug_request_payload(api_kwargs: Dict[str, Any], component: str = "ANTHROPIC") -> None:
        """
            ...
            if isinstance(tool, dict):
                ...
                if "description" in processed_tool and isinstance(processed_tool["description"], str):
                    ...
                    processed_tool["description"] = AnthropicDebugger._truncate_text_for_debug(
                        original_desc, max_length=100, field_name=f"tool '{tool.get('name', 'unknown')}' description"
                    )
                    ...
                    if "description" in prop_value and isinstance(prop_value["description"], str):
                        ...
                        processed_properties[prop_name] = {
                            "description": AnthropicDebugger._truncate_text_for_debug(
                                original_desc, max_length=80, field_name=f"parameter '{prop_name}' description"
                            )
                        }
                    else:
                        # If no description, still create entry but with placeholder
                        processed_properties[prop_name] = {"description": "<no description>"}
                else:
                ...
                if "content" in processed_msg:
                    ...
                    if isinstance(content, list):
                                ...
                                if content_block.get("type") == "text" and "text" in content_block:
                                    ...
                                    if isinstance(text_content, str) and len(text_content) > 150:
                                        processed_block["text"] = AnthropicDebugger._truncate_text_for_debug(
                                            text_content, max_length=150, field_name="message text"
                                        )
                        ...
                        processed_msg["content"] = AnthropicDebugger._truncate_text_for_debug(
                            content, max_length=150, field_name="message content"
                        )
    ...
    def _convert_object_to_dict(obj) -> Any:
        ...
        try:
            ...
            for attr in dir(obj):
                if not attr.startswith("_") and not attr.startswith("model_") and not callable(getattr(obj, attr)):
                    try:
        ...
        for i, msg in enumerate(messages):
            ...
            for j, content_block in enumerate(content):
                if not isinstance(content_block, dict):
                    print(
                        f"[{component}] WARNING: Message {i + 1} content block {j + 1} is not a dict: {type(content_block)}"
                    )
                    continue
